...
$(document).ready(function () {
    ...
    axios.get(`${backendserverurl}/genre/`)//getting genres for navbar
        .then((response) => {
            let genrehtmltoappend = "";
            response.data.forEach(genreinfo => {
                genrehtmltoappend += `<a class="dropdown-item navbargenre" href="javascript:void(0);" data-genreid="${genreinfo.genreid}">${genreinfo.genre}</a>`
            });
            $(".dropdown-menu").append(genrehtmltoappend);
            $('.navbargenre').on("click",function(){
                const searchtypeandstrreqdata = {"searchtype":"Genreid","searchstr":$(this).data("genreid")}
                axios.post(`${backendserverurl}/search/`,searchtypeandstrreqdata)
                .then((response) => {
                    if(response.data.length==0){
                        $("section").empty()
                        $("section").append(`<div class="row"><div class="col-lg-2"></div><h1 class="col-lg-8 text-center">No movie found for the genre<h1><div class="col-lg-2"></div></div>`);
                    }
                    else{
                        moviehtml(response)
                    }
    ...
    let token = localStorage.getItem("token");
    if(token){//check if login and if admin and put relevant nav links like admin page to add movies and genre and log out
        axios.get(`${backendserverurl}/auth/`,{headers: {"token":"Bearer " + token}})
        .then((response) => {
            if (response.data.type=="Admin"){
                $("#navbarul").append(`<li class="nav-item"><a class="nav-link" href="/admin">Admin page</a></li>`);
            }
            $("#navbarul").append(`<li class="nav-item"><a class="nav-link" id="logofflink" href="javascript:void(0);">Logoff</a></li>`);
            $('#logofflink').on("click",()=>{
        ...
        .catch((error)=> {
            $("#navbarul").append(`<li class="nav-item"><a class="nav-link" href="login">Login</a></li>`);
        });
    }
    else{
        $("#navbarul").append(`<li class="nav-item"><a class="nav-link" href="login">Login</a></li>`);
    };
    $("#searchform").submit(function() {//form to search for movie using title or genre
        event.preventDefault();
        let searchtypeandstrreqdata = {"searchtype":$("#searchtype").val(),"searchstr":$("#searchstr").val()};
        axios.post(`${backendserverurl}/search/`,searchtypeandstrreqdata)
        .then((response) => {
            if(response.data.length==0){
                $("section").empty()
                $("section").append(`<div class="row"><div class="col-lg-2"></div><h1 class="col-lg-8 text-center">No matching movie found<h1><div class="col-lg-2"></div></div>`);
            }
            else{
                moviehtml(response);
            }
